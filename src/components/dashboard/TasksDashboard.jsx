...
export default function TasksDashboard({ tasks = [], onComplete, onSelect }) {
        ...
        <div key={groupKey}>
          ...
          <div className="space-y-2">
            {groupedTasks[groupKey].map(task => (
              <div 
                key={task.id}
                className={`p-2 rounded-lg border transition-colors cursor-pointer ${groupKey === 'scaduti' ? 'bg-red-50 border-red-200 hover:border-red-300' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}
